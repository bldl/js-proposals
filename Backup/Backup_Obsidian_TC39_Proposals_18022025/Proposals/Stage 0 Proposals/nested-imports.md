...
6. fetch the source for the entry point module;
7. scan it for requested module identifier strings (which does not require a full parse!);
8. resolve those requested module identifiers relative to the parent module;
9. asynchronously fetch the sources of the requested modules (in parallel);
10. scan the requested modules for additional dependencies;
11. fetch their sources in parallel;
12. etc. etc. until closure reached; and
13. only then begin evaluating the entry point module, with full confidence
