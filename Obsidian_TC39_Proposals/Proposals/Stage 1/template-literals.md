[[Stage 1]]<br>Classification: [[Syntactic Change]]<br>Human Validated: KW<br>Title: Improved Escapes for Template Literals<br>Authors: HE Shi-Jun<br>Champions: HE Shi-Jun<br>Last Presented: February 2024<br>Stage Upgrades:<br>Stage 1: 2024-08-19 
Stage 2: NA  
Stage 2.7: NA  
Stage 3: NA  
Stage 4: NA<br>Last Commit: 2024-08-19<br>Topics: #others<br>Keywords: #string #escape #raw_string_literal<br>GitHub Link: https://github.com/hax/proposal-raw-string-literals <br>GitHub Note Link: https://github.com/tc39/notes/blob/HEAD/meetings/2024-02/feb-7.md#raw-string-literals-for-stage-1
...
2. Current usages of template literals should be easily migrate to the new syntax. Importantly, if a template literal does not need to present `${` characters, it should not be forced to change interpolation syntax.
3. Nice to have mechnism like [Markdown info string](https://spec.commonmark.org/0.30/#info-string), which can be leavaged by tools (for example, use for syntax highlighting).
4. Nice to have mechnism to do comments. (though people could abuse interpolations for that purpose)
5. Nice to have mechnism to enable escaping in specified place. (Swift support that.)
6. To nest JS code, it will be better if the syntax allow short delimeter outside, long delimeter inside. So when you write nested JS code, you don't need to go back to the start and change the delimiter. This is also help LLM AI to generate JS code, because current LLM can't backtrace and modify the codes they already output.
