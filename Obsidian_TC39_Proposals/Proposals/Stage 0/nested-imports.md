[[Stage 0]]<br>Classification: [[Syntactic Change]] [[Semantic Change]]<br>Human Validated: KW<br>Title: Nested import declarations<br>Authors: Ben Newman<br>Champions: Ben Newman<br>Last Presented: July 2016<br>Stage Upgrades:<br>Stage 1: NA 
Stage 2: NA  
Stage 2.7: NA  
Stage 3: NA  
Stage 4: NA<br>Last Commit: 2016-07-24<br>Topics: #modules #performance<br>Keywords: #module #declaration #lazy #performance <br>GitHub Link: https://github.com/benjamn/reify/blob/HEAD/PROPOSAL.md <br>GitHub Note Link: https://github.com/tc39/notes/blob/HEAD/meetings/2016-07/jul-27.md#10iiic-nested-import-declaration
...
2. **hoisted** to the beginning of the enclosing scope; that is, _declarative_ rather than _imperative_;
...
2. scan it for requested module identifier strings (which does not require a full parse!);
3. resolve those requested module identifiers relative to the parent module;
4. asynchronously fetch the sources of the requested modules (in parallel);
5. scan the requested modules for additional dependencies;
6. fetch their sources in parallel;
7. etc. etc. until closure reached; and
8. only then begin evaluating the entry point module, with full confidence
